...
## 1️. Retrieve All Network Connection Events
...
## 3️. Detect Browser-Initiated Connections
```spl
...
## 4️. Detect Connection to Attacker IP
```spl
...
## 5️. High-Confidence Phishing Click Detection
```spl
...
## 6️. Monitor Outbound Web Traffic (HTTP/HTTPS)
```spl
...
## 8️. Identify Most Contacted IP Addresses
```spl
index=sysmon EventCode=3
